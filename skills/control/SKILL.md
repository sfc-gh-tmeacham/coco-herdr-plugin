...
> **Cortex Code note.** Herdr does not recognize `cortex` as a native agent kind.
> This plugin reports CoCo's state over the socket API instead (source
> `custom:coco`, label `coco`). `herdr agent list` therefore shows CoCo panes, but
> `herdr agent explain` and `herdr agent start --kind` do not apply to them.
> Target CoCo panes by pane ID. Source text below is Herdr's own skill, printed
> by `herdr --skill`, bundled unchanged.
